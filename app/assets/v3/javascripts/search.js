const getSearchSuggestions = async (searchString) => {
	return new Promise((resolve, reject) => {
		...
		xhr.open('GET', `/v3/suggestions?searchString=${searchString}`, true)
			...
			if(suggestions?.count) {
				...
				suggestions.rows.forEach(row => {
					...
					domSuggestions.append(getOption(val, val,
						() => window.location.href = `${window.location.origin}/v3/details?payeeName=${row.payee_name}&partPostcode=${row.part_postcode}&searchString=${searchInput.value}`,
						(event) => {
